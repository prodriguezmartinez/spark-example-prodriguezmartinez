...
public class MockitoTests {
	...
	public void MockListSize() {
		List list = mock(List.class); //se da el cambiazo en vez de new list 
		when(list.size()).thenReturn(10); //cuando llames a list.size devuelves 10 (no existe ninguna lista)
	...
	public void MockListSizeWithMultipleReturnValues() {
		...
		when(list.size()).thenReturn(10).thenReturn(20); //si llaman 1 vez list.size es 10 y la 2 y sigientes 20.
		...
		verify(list, times(3)).size(); //mockito guarda cuantas veces se ha llamado a list desde que se creo.
	}
